...
export default function UsersPage() {
    ...
    const handleDeleteUser = async () => {
        ...
        try {
            ...
            setAlert({ type: "error", message: "Falha ao excluir o usuário." });
        } finally {
                ...
                <div className="my-4 w-full flex flex-row-reverse">
                    <div className="flex flex-wrap gap-2 items-center">
                        {activeFilters.map((filter, index) => (
                            ...
                            >
                                {getColumnLabel(filter.column)} {getOperatorLabel(filter.operator)} "{filter.term}"
                                <button 
                            ...
                            <tr key={user.id}>
                                ...
                                <td>
                                    {user.classes.length > 0 ? (
                                        user.classes.map((cls, idx) => <div key={idx}>{cls}</div>)
                                    ) : (
                                        <span>Nenhuma</span>
                                    )}
                                </td>
                                <td>
                                    {user.courses.length > 0 ? (
                                        user.courses.map((course, idx) => <div key={idx}>{course}</div>)
                                    ) : (
                                        <span>Nenhum</span>
                                    )}
                                </td>
