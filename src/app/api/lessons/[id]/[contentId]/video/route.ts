  ...
  try {
    ...
    const [hasAccess]: any = await pool.query(
      `SELECT 1
       ...
       WHERE lc.id = ? AND l.id = ? AND cu.user_id = ? AND lc.content_type = 'video'`,
      [contentId, lessonId, userId]
    );

    if (!hasAccess || hasAccess.length === 0) {
      return NextResponse.json({ message: "Você não tem acesso a este conteúdo ou o tipo de conteúdo não é vídeo" }, { status: 403 });
    }
    ...
    // Obter conteúdo de vídeo
    const [contentRows]: any = await pool.query(
      `SELECT lv.id, lv.lesson_content_id, lv.video_title, lv.video_url, lv.video_content, lv.created_at
       FROM LessonVideo lv
       JOIN LessonContents lc ON lv.lesson_content_id = lc.id
       WHERE lc.id = ? AND lc.content_type = 'video'`,
      [contentId]
    );

    if (contentRows.length === 0) {
      return NextResponse.json({ message: "Conteúdo de vídeo não encontrado" }, { status: 404 });
    }

    // Obter conteúdos adjacentes para navegação prev/next
