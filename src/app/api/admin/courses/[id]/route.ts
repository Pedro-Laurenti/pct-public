...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const [rows] = await pool.query<RowDataPacket[]>(
            "SELECT id, name, description, cover_image, COALESCE(price, 0.00) AS price, COALESCE(is_active, 1) AS is_active FROM Courses WHERE id = ?",
            [id]
        );
...
export async function PUT(request: NextRequest) {
    ...
    try {
        ...
        const [result] = await pool.query(
            "UPDATE Courses SET name = ?, description = ?, price = ?, is_active = ?, cover_image = ? WHERE id = ?",
            [name, description, price ?? 0, is_active ?? 1, cover_image || null, id]
        );
