...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!hasAccess || hasAccess.length === 0) {
      return res.status(403).json({ message: "Você não tem acesso a este conteúdo ou o tipo de conteúdo não é vídeo" });
    }
    ...
    if (contentRows.length === 0) {
      return res.status(404).json({ message: "Conteúdo de vídeo não encontrado" });
    }
